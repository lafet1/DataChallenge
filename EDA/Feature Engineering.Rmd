...
This is experiment. What is the share of words which are correctly spelled, and also in English dictionary in the document?


```{r}

correct_words<-c()
for (i in subdf$description){
  correct_words<-c(correct_words, how_many_correct(i))
}
...
abc<-sapply(subdf$description, how_many_correct)
```
...
```{r, eval = FALSE}
write_feather(subdf, path = "data.feather")
writeMM(dtm, file="dtm.txt")
writeMM(dtm_title, file="dtm_title.txt")
writeMM(dtm_tfidf, file="dtm_tfidf.txt")
writeMM(dtm_title_tfidf, file="dtm_title_tfidf.txt")
```
