...
results <- matrix(ncol = 2, nrow = 25)
for (i in 1:25){
  history <- nn_varepochs %>% fit(
    x_train, y_train, shuffle = T,
    epochs = 2, batch_size = 2048, 
    validation_set = 0.2)
...
cbind(results, (1:25) * 2)
colnames(results) <- c("sensitivity", "specificity", "epochs")
results <- results %>% as.data.frame() %>% as.tbl() %>%
  ggplot(aes(x = epochs)) + geom_line(aes(y=sensitivity)) + geom_line(aes(specificity))
