...
does not suppress strong signals, i.e. its range is from zero to infinity.
It can be improved as there is a Leaky ReLU that has range from minus to plus infinity.
This also means that not as many neurons die, i.e. if the activation is zero in neuron then it can be ignored during the training and "die". Leaky ReLU solves this.
...
precision/recall curve is more appropriate for this task.
In the end one must choose the measure that is good for the individual task at hand, 
...
# Exercise 6
Epoch is one iteration of the optimizer (i.e. SGD or in our case rmsprop) algorothm, 
i.e. In one epoch your algorithm sees every data point exactly once.
...
theoretical number of batches is 1, i.e. all data points at once, or N, i.e. every data point alone, which is the so-called online SGD.
