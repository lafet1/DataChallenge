...
task.under = undersample(task.over, rate = 1/7)
...
model %>% 
  ...
  layer_dense(units = 32, activation = layer_activation_leaky_relu(alpha = 0.3)) %>%
  layer_dropout(rate = 0.3) %>%
  layer_dense(units = 16, activation = layer_activation_leaky_relu(alpha = 0.1)) %>%
  layer_dropout(rate = 0.3) %>%
  layer_dense(units = 2, activation = 'softmax')

class_weight = list('0' = 1, '1' = 1.6)
